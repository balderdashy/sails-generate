...
module.exports = function loadGeneratorDef(options) {
  ...
  if (options.modules && !_.isUndefined(options.modules[options.generatorType])) {
    ...
    if (!_.isString(options.modules[options.generatorType])) {
      return cb.invalid(new Error('Invalid package name/module path configured as `options.modules[\''+options.generatorType+'\']`.  Should be a string, but instead got: '+util.inspect(options.modules[options.generatorType], {depth: null})));
    }

    generatorModulePath = options.modules[options.generatorType];

  }

  // Set up a regexp for use below.
  var CONTAINS_GENERATOR_MODULE_PATH_RX = new RegExp(_.escapeRegExp(generatorModulePath));

  // Attempt to require the generator definition in the following ways:
  // > (in descending order of priority)
  var requiresToAttempt = [
    path.resolve(__dirname, './core-generators', options.generatorType),// as absolute path resolved from THIS PACKAGE'S core-generators/
    'sails-generate-'+generatorModulePath, // as package name -- i.e. a normal `require()`, from any global deps installed in `${HOME}` (and technically from sails-generate's own dependencies, but that _should_ never actually be relevant nowadays, unless something really weird is going on)
    path.resolve(process.cwd(), generatorModulePath),// as absolute path resolved from `${CWD}/sails-generate-<<generatorType>>`
    path.resolve(options.rootPath, 'node_modules', 'sails-generate-'+options.generatorType),// as absolute path resolved from `<<rootPath>>/node_modules/sails-generate-<<generatorType>>`
    path.resolve(process.cwd(), 'node_modules', generatorModulePath),// as absolute path resolved from `${CWD}/node_modules/sails-generate-<<generatorType>>`
    path.resolve(options.rootPath, generatorModulePath),// as absolute path resolved from `<<rootPath>>/sails-generate-<<generatorType>>` (i.e. to allow for relative paths)
  ];

  // If an explicit generator package name could be deduced, add some additional requires.
  if (generatorModulePath) {
    // TODO
  }
