...
module.exports = function loadGeneratorDef(options) {
  ...
  //
  // > In the past, we would prefix with `sails-generate-` here-- but nowadays,
  // > this is just exactly the same as the `generatorType`.
  else {

    generatorPkgName = options.generatorType;
  ...
  var requiresToAttempt = [
    path.resolve(__dirname, './core-generators', generatorPkgName),// package name as absolute path resolved from THIS PACKAGE'S core-generators
    generatorPkgName, // package name -- i.e. a normal `require()`, from any global deps installed in `${HOME}` (and technically from sails-generate's own dependencies, but that _should_ never actually be relevant nowadays, unless something really weird is going on)
