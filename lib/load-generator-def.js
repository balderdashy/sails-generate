...
module.exports = function loadGeneratorDef(options) {
  ...
  var requiresToAttempt = [
    ...
    path.resolve(options.rootPath, 'node_modules', generatorPkgName),// from the rootPath
    path.resolve(process.cwd(), 'node_modules', generatorPkgName),// from console user's cwd
    path.resolve(options.rootPath || process.cwd(), generatorPkgName),// from sails-generate's dependencies
  ];
