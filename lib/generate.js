	...
	Generator.before(scope, reportback.extend({
		...
		success: function() {
			...
			function done(err) {
				...
				if (err) {
					var errorFn = Generator.error || function defaultError(err, _cb, scope) {
						return _cb(err);
					};
					return errorFn(err, sb, scope);
				}

				// Expose a `after` handler in generators (on success only)
				var afterFn = Generator.after || function defaultAfter(_cb, scope) {
					return _cb();
				};
				return afterFn(sb, scope);
