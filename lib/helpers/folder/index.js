...
module.exports = function ( options, handlers ) {
	...
	handlers = switchback(handlers, { error: handlers.error });
	...
	var destPath = path.resolve( process.cwd() , options.destPath );


	// Only override an existing folder if `options.force` is true
	fs.lstat(destPath, function(err, inodeStatus) {
		var exists = !(err && err.code === 'ENOENT');
		if (exists && err) return handlers.error(err);

		if (exists && !options.force) {
			return handlers.alreadyExists(destPath);
		}
		if (exists) {
			fs.remove(destPath, function deletedOldINode(err) {
		...
		function _afterwards_() {
			...
			fs.mkdirs(destPath, function directoryWasWritten(err) {
				if (err) return handlers.error(err);
