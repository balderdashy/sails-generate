...
var EjsHelper = require('./helpers/ejs');
...
function generateTarget (options) {
	...
	async.until(function checkIfTargetIsValidYet () {
		...
		if (target.ejs) {
			scope = mergeSubtargetScope(scope,typeof target.ejs === 'string' ? {templatePath: target.ejs} : target.ejs); 
			return EjsHelper(scope, cb);
		}
		if (target.jsonfile) {
			if (typeof target.jsonfile === 'object') {
				...
				scope = _.merge(scope, target.jsonfile(scope));
			}
 ...
 * @type {Array}
 */
var KNOWN_HELPERS = ['exec', 'folder', 'ejs', 'jsonfile', 'file', 'copy'];
