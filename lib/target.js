 ...
 * @param  {Object}          parent      parent generator (if applicable)
 * @return {[type]}                     [description]
 */
function parseTarget(target, scope, cb, parent) {
  ...
  if (target.generator) {
    ...
    var SAILS_GENERATE_PREFIX = 'sails-generate-';
    var requireErrorMessages = [
      'Generator Error: Failed to load "',
      subGeneratorRef.module,
      '"...'
    ];

    var subGeneratorRequireStrategies = [
      function fromPath() {
        return require(module);
      },
      function fromScopeRootPath() {
      ...
      function fromGlobalGeneratorPrepend() {
        return require(SAILS_GENERATE_PREFIX + module);
      },
      function fromParentGeneratorModules() {
        return require(path.resolve(parent.moduleDir, 'node_modules', module));
      },
      function fromParentGeneratorModulesPrepend() {
        return require(path.resolve(parent.moduleDir, 'node_modules', SAILS_GENERATE_PREFIX +  module));
      }
    ];

    // Loop through strategies until one works
    _(subGeneratorRequireStrategies).each(function findSubGenerator(strategy) {
      if (subGenerator) {
        return;
      }
      try {
        subGenerator = strategy();
      } catch (ex) {
        requireErrorMessages.push('\n\tTried: ' + strategy.name + '(), threw: ' + ex.message);
      }
    });
    ...
    return cb(new Error(requireErrorMessages.join('')));
