...
function generateTarget (options) {
	...
	async.until(function checkIfTargetIsValidYet () {
		...
		if (++scope._depth > scope.maxHops) {
			return cb(new Error('`maxHops` ('+scope.maxHops+' exceeded!  There is probably a recursive loop in one of your generators.'));
		}
	...
	if (target.generator) {
		...
		if (!subGeneratorRef.module) {
			...
			try {
				...
				// If we didn't find that submodule, try "sails-generate-<module>"
				try {
					subGenerator = require('sails-generate-'+subGeneratorRef.module);
				}
