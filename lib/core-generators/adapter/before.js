...
// The default semver range to use when generating files related to
// Waterline and WAT (waterline-adapter-tests) compatibility.
var WATERLINE_SVR = '^0.13.0';
...
module.exports = function(scope, cb) {
  ...
  _.defaults(scope, {
    ...
    // Waterline semver range
    waterlineSVR: WATERLINE_SVR
  ...
  _.defaults(scope, {
    ...
    repository: _.defaults(scope.repository || {}, {
      type: 'git',
      url: util.format('git://github.com/%s/sails-%s.git', scope.github.username)
    })
  });
  ...
  if (scope.github.username) {
    scope.packageName = ('@'+scope.github.username+'/' + 'sails-'+scope.adapterType).toLowerCase();
  }
  else {
    scope.packageName = ('sails-'+scope.adapterType).toLowerCase();
  }
