...
module.exports = function before (scope, done) {
  ...
  //
  // ```
  // cd .. && rm -rf minimal && sails new minimal --fast --caviar && echo 'cd-ing in...' && cd minimal
  // ```
  //
  // ```
  // cd .. && rm -rf basic && sails new basic --fast --caviar && echo 'cd-ing in...' && cd basic
  // ```
  //
  // ```
  // cd .. && rm -rf expanded && sails new expanded --fast --caviar && echo 'cd-ing in...' && cd expanded
  // ```
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  if (_.contains(scope.without, 'session')) {
    ...
    scope.modules['api/policies/isLoggedIn.js'] = false;
    scope.modules['api/policies/isAdmin.js'] = false;
  }//-•
