...
module.exports = function before (scope, done) {
  ...
  if (scope.caviar) {
    console.log('this is caviarl');
    // `caviar` takes precedence.
    ...
    console.log('this is traditional');
    scope.modules['api/models/User.js'] = false;
  }
  ...
  // Loop over the targets in this generator, determining which ones are flagged
  // with `onlyIf` -- and omit the target, if appropriate.
  //
  // > For example, if this generator is being run with `--caviar`, then still
  // > generate all `onlyIf: 'caviar'` targets.  But otherwise, rip them out.
  var targetsToDisable = [];
  (function $recurse(targets, pathSoFar){
    _.each(targets, function(rhs, key){
      if (_.isArray(rhs)) {
        _.each(rhs, function(item) {
          if (_.isObject(item) && item.generator) {
            $recurse(item.generator.targets, path.join(pathSoFar, key));
          }
        });//∞
      }
      else if (_.isObject(rhs) && rhs.generator) {
        $recurse(rhs.generator.targets, path.join(pathSoFar, key));
      }
      else if (_.isObject(rhs) && rhs.onlyIf !== undefined && !scope[rhs.onlyIf]) {
        targetsToDisable.push(path.join(pathSoFar, key));
      }
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      // FUTURE: Maybe support sexier expressions for onlyIf.
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    });//∞
  })(this.targets, '');
  // console.log('*', targetsToDisable);

  // Set each matching target key as `false` in `scope.modules`.
  // (This will cause them to be disabled when the main generator runs.)
  _.each(targetsToDisable, function(targetKey){
    scope.modules[targetKey] = false;
  });
  // console.log('----', util.inspect(scope.modules,{depth:null}));
