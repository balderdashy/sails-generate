...
module.exports = function getPackageJsonData(scope) {
  ...
  if(scope.caviar && !IS_CURRENT_NODE_VERSION_CAPABLE_OF_AWAIT) {
    ...
    console.error('------------------------------------------------------------------------------------------');
    console.error('It looks like your current version of Node.js is '+process.version+'.');
    console.error('We\'re sorry, but to build with Sails 1.0\'s expanded starter app, you must use');
    console.error('Node version 7.9 or greater.  Please upgrade your version of Node, or generate');
    console.error('a basic Sails app instead.');
    console.error();
    console.error(' [?] If you\'re unsure or want advice, swing by https://sailsjs.com/support');
    console.error('------------------------------------------------------------------------------------------');
    throw flaverr({
      name: 'CompatibilityError',
      code: 'E_NODE_VERSION_TOO_LOW',
      message: 'Node version too low (needs Node v7.9 or greater).'
    });
  }
