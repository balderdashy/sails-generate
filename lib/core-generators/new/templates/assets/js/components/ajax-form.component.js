    ...
    submit: async function () {
      ...
      if (!this.action) {
        ...
        if (!_.isString(this.action) || !_.isFunction(Cloud[_.camelCase(this.action)])) {
          ...
          throw new Error('Unrecognized `action` in <ajax-form>.  Did you mean to type `action="'+_.camelCase(this.action)+'"`?  (`action` should always be specified in camelCase.)');
        }
        ...
        console.log('ABOUT TO HANDLE PARSING');
        var argins;
        try {
        ...
        console.log('DONE HANDLING PARSING',argins);
        ...
        await Cloud[this.action](argins)
        .tolerate((err)=>{
        ...
        // If the server says wer were successful, then emit the "after-submitting" event.
        if (!didResponseIndicateFailure) {
          this.$emit('after-submitting');
        }
