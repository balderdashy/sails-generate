...
parasails.registerUtility('isValidEmailAddress', function(options) {

  if(!_.isString(options.string)) {
    throw new Error('Usage error: `string` must be provided as a valid string!  But instead got `'+options.string+'` (a "'+typeof options.string+'")');
  }
  ...
  if(options.string.match(doesntStartWithAlphanumericRX) || options.string.match(doesntEndWithAlphanumericRX)) {
    return false;
  }
  ...
  var chunksOnEitherSideOfAtSymbol = options.string.split('@');
