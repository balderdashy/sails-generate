parasails.registerPage('account-overview', {
  ...
  methods: {
    ...
    clickStripeCheckoutButton: async function() {
      ...
      var billingCardInfo;
      try {
        billingCardInfo = await openStripeCheckout(this.stripePublishableKey, this.me.emailAddress);
      } catch (err) {
        if (err.code === 'E_COULD_NOT_OBTAIN_STRIPE_TOKEN') {
          // If there were any issues such that a Stripe token couldn't be obtained,
          // then just immediately set our Stripe error flag and avast (don't proceed
          // any further.)
          this.stripeError = true;
      ...
      // After payment info has been successfully added, update the billing info
      // for this user in our backend.
      this.syncingUpdateCard = true;
