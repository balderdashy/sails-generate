module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    var stripeCustomerId = await sails.stdlib('stripe').saveBillingInfo({
      stripeCustomerId: this.req.me.stripeCustomerId,
      token: inputs.stripeToken || '',
    });

    // Update the card info we have stored for this user in our database.
    ...
    .set({
      ...
      billingCardExpMonth: inputs.billingCardExpMonth || 0,// todo: check back on this.  why numbers?
      billingCardExpYear: inputs.billingCardExpYear || 0// todo: check back on this.  why numbers?
    });
