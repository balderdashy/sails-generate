...
module.exports = {
  ...
  before: function (scope, done){
    ...
    // Infer description, if relevant.
    if (_.isUndefined(scope.description)) {

      var inferredDescription;
      ...
      var chunks = scope.actionName.split('.');
      var lastChunk = _.last(chunks);
      var actionVerb = _.words(lastChunk)[0];
      if (_.words(lastChunk).length === 1 && chunks.length === 1) {
        scope.description = _.capitalize(actionVerb) + ' something.';
      }
        ...
        var midSentenceCaseAkkusativ = _.map(_.words(akkusativ), function (word, i){ return word[0].toLowerCase() + word.slice(1); }).join(' ');
        scope.description = _.capitalize(actionVerb)+' '+midSentenceCaseAkkusativ+'.';
      }
      else {
  ...
  after: function (scope, done) {
    if (scope.actions2) {
      ...
      done.log.debug('(see http://sailsjs.com/docs/concepts/actions)');
    }
    return done();
  },
