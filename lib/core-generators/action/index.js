...
module.exports = {
  ...
  before: function (scope, done){
    ...
    if (stem.match(/^view/i) && stem !== 'view') {
      ...
      var inferredViewTemplatePath = path.join('pages/', intermediateSubdirs.join('/'), inferredViewTemplateBasename);
  ...
  after: function (scope, done) {
    ...
    if (scope.actions2) {
      done.log.info('Created an action!');
      console.log('Using "actions2"...');
      console.log(' [?] https://sailsjs.com/docs/concepts/actions');
    }
    else {
      done.log.info('Created a traditional (req,res)-style action.');
    }
    return done();
  },
