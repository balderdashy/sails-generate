...
module.exports = {
  ...
  before: function (scope, exits) {
    ...
    if (stem.match(/^view-/)) {
      return exits.error('No need to prefix with "view-".  Instead, just leave that part out.  (It\'ll be added automatically where needed.)');
    }
  ...
  after: function (scope, done){
    ...
    console.log('          \'GET /'+scope.relPath+'\': { action: \''+scope.newActionSlug+'\' }');
    console.log();
    ...
    console.log('          @import \''+scope.newStylesheetRelPath+'\';');
    console.log();
...
'  <p>(See also <em>'+scope.newStylesheetRelPath+'</em>, <em>'+scope.newPageScriptRelPath+'</em>, and <em>'+scope.newActionRelPath+'</em>.)</p>\n'+
'\n'+
'</div>\n'+
'<%- /* Expose server-rendered data as window.SAILS_LOCALS :: */ exposeLocalsToBrowser() %>\n'+
'';
        if (fs.existsSync(scope.rootPath) && !scope.force) {
          return done(new Error('A file already exists at '+scope.rootPath+'.  Use --force to override.'));
        }
        fs.writeFileSync(scope.rootPath, contents);
        return done();
      }
    },
