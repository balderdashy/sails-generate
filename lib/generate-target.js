...
module.exports = function generateTarget(options, done) {
  ...
  async.until(
    ...
    function check() {
      ...
      // (But only if we've checked it for overrides AT LEAST ONCE)
      if (isThisTargetValid && hasBeenLookedAtOnce) {
        return true;
      }//-•
    ...
    function iteratee(proceed) {
      ...
      if (target.overridable) {
        ...
        try {
          ...
          console.log('found generator for `'+scope.rootPath+'`',target.generator);
      ...
      // Flag this as having been checked for overrides.
      console.log('Flagging',scope.rootPath);
      hasBeenLookedAtOnce = true;
      ...
      if (!_.isUndefined(target.overidable) || !_.isUndefined(target.overriddable) || !_.isUndefined(target.overiddable) || !_.isUndefined(target.overideable) || !_.isUndefined(target.overrideable) || !_.isUndefined(target.overiddeable)) {
        return proceed(new Error('Unrecognized target syntax: Please check your spelling.  Did you mean "overridable"?'));
      }
      ...
      if (!_.isPlainObject(target.generator)) {
        return proceed(new Error('Generator error: Invalid sub-generator configuration for target "' + scope.rootPath + '": '+util.inspect(target.generator, {depth:null})));
      }
