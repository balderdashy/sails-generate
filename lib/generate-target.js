...
module.exports = function generateTarget(options, cb) {
  ...
  async.until(
    ...
    function afterwards(err) {
      ...
      if (target.copy) {
        ...
        return generateFileCopy(scope, sb);
      }
      if (target.folder) {
        scope = mergeSubtargetScope(scope, target.folder);
        return generateFolder(scope, sb);
      }
      if (target.template) {
        ...
        return generateFileFromTemplate(scope, sb);
      }
      if (target.jsonfile) {
        ...
        return generateJsonFile(scope, sb);
      }
      ...
      return helpGenerateRecursive(target, scope, sb);
