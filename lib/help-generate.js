...
module.exports = function helpGenerate(Generator, scope, cb) {
  ...
  if (!scope.rootPath) {
    return sb(new Error('Consistency violation: Missing scope variable: `rootPath`\nPlease make sure it is specified and try again.  If you are seeing this message, one or more custom generators installed on this computer may have an internal error.  If there are no custom generators installed in Sails, this could be indicative of a bug or version compatibility / `npm link` issue with the globally-installed Sails generators.  If you need ruther help, visit http://sailsjs.com/support.'));
  }
  ...
  Generator.before(scope, reportback.extend({
    ...
    success: function() {
      ...
      async.each(Object.keys(Generator.targets), function (keyPath, next) {
        var async_each_sb = reportback.extend(next);
